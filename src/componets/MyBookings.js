...
export default function MyBookings() {
  ...
  const fetchBookings = useCallback(async (silent = false) => {
    ...
    try {
      ...
      setBookings(data);
    } catch { if (!silent) showToast('Failed to load bookings', 'error'); }
  ...
  const handleCancel = async booking => {
    if (!window.confirm(`Cancel appointment with Dr. ${booking.doctor_name}?`)) return;
    setCancelling(booking.id);
    try {
      await API.patch(`/bookings/cancel/${booking.id}/`);
      await fetchBookings(true);
      showToast('Appointment cancelled successfully');
    } catch (err) { showToast(err?.response?.data?.message || 'Failed to cancel booking.', 'error'); }
    finally { setCancelling(null); }
  };

  const openReschedule = async booking => {
    setRescheduleBooking(booking); setNewDate(''); setNewSlot('');
    try {
  ...
  const handleReschedule = async () => {
    ...
    } catch (err) { showToast(err?.response?.data?.message || 'Reschedule failed.', 'error'); }
  ...
  return (
    <>
      <style>{`
        ...
        .mb-tab-badge {
          display: inline-block; margin-left: 6px; background: var(--blue-100); color: var(--blue-700);
        ...
        .mb-refresh.spinning svg { animation: spin 0.9s linear infinite; }
        ...
        .mb-token-col {
          ...
          border-right: 1px solid var(--blue-50); background: var(--blue-50);
        }
        ...
        .mb-token-num {
          ...
          font-size: 1.4rem; font-weight: 500; color: var(--blue-700);
        ...
        /* Queue panel */
        .mb-queue-panel {
          border-top: 1px solid var(--blue-50); padding: 14px 20px;
          display: flex; align-items: center; gap: 14px; flex-wrap: wrap;
          background: #F0F9FF;
        }
        .mb-queue-circle {
          width: 48px; height: 48px; border-radius: 50%;
          background: var(--blue-50); border: 2px solid var(--blue-300);
          display: flex; align-items: center; justify-content: center;
          font-family: 'Plus Jakarta Sans', sans-serif; font-size: 1.2rem; font-weight: 800; color: var(--blue-600);
          flex-shrink: 0;
        }
        ...
        /* Action panels */
        .mb-action-panel {
          border-top: 1px solid var(--blue-50); padding: 12px 20px;
        ...
        .mb-cancel-btn {
          ...
          color: var(--color-error-text); cursor: pointer; transition: all 0.15s;
        }
        ...
        .mb-reschedule-btn {
          ...
          border-radius: 9px; padding: 8px 16px;
          font-family: 'DM Sans', sans-serif; font-size: 13px; font-weight: 600;
          color: var(--blue-700); cursor: pointer; transition: all 0.15s;
        }
        ...
        .mb-modal-overlay {
          ...
          background: rgba(4,44,83,0.45); backdrop-filter: blur(6px);
        ...
        .mb-modal-input {
          ...
          border-radius: 11px; padding: 11px 14px; font-family: 'DM Sans', sans-serif;
          font-size: 15px; color: var(--gray-900); outline: none; transition: all 0.15s;
          color-scheme: light; margin-bottom: 18px;
        }
        ...
        .mb-slot-btn {
          padding: 8px 4px; border-radius: 9px; border: 1px solid var(--blue-100);
          background: var(--gray-50); font-size: 12px; font-weight: 500;
          color: var(--gray-600); cursor: pointer; transition: all 0.15s; text-align: center;
        ...
        .mb-modal-cancel {
          flex: 1; padding: 12px; border-radius: 11px; border: 1px solid var(--blue-100);
        ...
        .mb-modal-confirm {
          ...
          cursor: pointer; transition: all 0.15s; box-shadow: 0 4px 12px rgba(24,95,165,0.2);
        }
        ...
        @media (max-width: 600px) {
          .mb-token-col { width: 84px; padding: 16px 10px; }
          .mb-info-col  { padding: 14px; }
          .mb-action-panel { padding: 11px 14px; }
        }
      `}</style>
        ...
        <div className="tw-container" style={{ paddingTop: 32 }}>
          ...
          {loading && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
              {[...Array(3)].map((_, i) => <div key={i} className="mb-skel"><div className="mb-skel-shine" /></div>)}
            </div>
          )}
                    ...
                    <div className="mb-card-top">
                      ...
                      <div className="mb-info-col">
                        <div style={{ marginBottom: 8 }}>
                          <span className={`badge ${st.cls}`} style={{ marginBottom: 8, display: 'inline-flex' }}>
                      ...
                      <div className="mb-queue-panel">
                        ...
                        <div>
                          ...
                          <div className="mb-queue-desc">
                            {booking.queue_position === 1 ? '🎉 You\'re next! Head to the clinic'
                              : booking.queue_position === 0 ? '✅ Your turn — please go in'
                              : `${(booking.queue_position || 1) - 1} patient${booking.queue_position > 2 ? 's' : ''} ahead of you`}
                          </div>
                        </div>
                        <div style={{ fontSize: 12, color: 'var(--gray-400)', marginLeft: 'auto' }}>Auto-refreshes every 15s</div>
                      </div>
                    )}
      ...
      {rescheduleBooking && (
        <div className="mb-modal-overlay" onClick={e => { if (e.target === e.currentTarget) setRescheduleBooking(null); }}>
          <div className="mb-modal">
            ...
            <input type="date" className="mb-modal-input" min={today} value={newDate}
