  ...
  return (
    <div className="container-fluid p-4">
        ...
        {hospitals.slice(0, 4).map(h => (
          <div key={h.id} className="col-md-3">
            <div className="card border-0 shadow-sm p-3">
              ...
              <div className="mt-2">
                <span className={`badge ${h.status === 'active' ? 'bg-success' : 'bg-secondary'}`}>{h.status}</span>
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="d-flex justify-content-between align-items-center mb-3">
        ...
        <input type="text" placeholder="Search doctors…" className="form-control" style={{ maxWidth: 280 }}
        ...
        <div className="card shadow-sm border-0">
          <table className="table table-hover mb-0">
            <thead className="table-light">
              <tr><th>Name</th><th>Specialization</th><th>Hospital</th><th>Exp</th><th>City</th><th>Fee</th><th>Status</th><th>Actions</th></tr>
            </thead>
            <tbody>
              {filtered.length === 0 && (
              ...
              {filtered.map(doc => (
                <tr key={doc.id}>
                  ...
                  <td>{doc.city}</td>
                  <td>₹{doc.fee}</td>
                  <td><span className={`badge ${doc.available ? 'bg-success' : 'bg-secondary'}`}>{doc.available ? 'Available' : 'Unavailable'}</span></td>
                  <td>
                    <button className="btn btn-primary btn-sm me-2" data-bs-toggle="modal" data-bs-target="#editModal" onClick={() => openEdit(doc.id)}>Edit</button>
                    <button className="btn btn-danger btn-sm" onClick={() => deleteDoctor(doc.id)}>Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
                ...
                <form onSubmit={submitEdit}>
                  {['name', 'specialization', 'city', 'experience', 'fee'].map(field => (
                    <div className="mb-2" key={field}>
                      ...
                      <input name={field} value={editDoctor[field] || ''}
                        onChange={e => setEditDoctor(prev => ({ ...prev, [e.target.name]: e.target.value }))}
                        className="form-control" placeholder={field} />
                    </div>
                  ))}
                  <div className="mb-3">
                    <label className="form-label">Available</label>
                    <select className="form-select" value={editDoctor.available ? 'true' : 'false'}
                      onChange={e => setEditDoctor(prev => ({ ...prev, available: e.target.value === 'true' }))}>
