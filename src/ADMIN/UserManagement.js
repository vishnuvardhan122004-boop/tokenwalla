        ...
        <div className="card shadow-sm border-0">
          <table className="table table-hover mb-0">
            <thead className="table-light">
              <tr><th>#</th><th>Name</th><th>Mobile</th><th>Role</th><th>Status</th><th>Action</th></tr>
            </thead>
            <tbody>
              {filtered.length === 0 && (
              ...
              {filtered.map((user, i) => (
                <tr key={user.id}>
                  ...
                  <td><span className={`badge ${user.status === 'blocked' ? 'bg-danger' : 'bg-success'}`}>{user.status || 'active'}</span></td>
                  <td>
                    {user.status === 'blocked'
                      ? <button className="btn btn-success btn-sm" onClick={() => activateUser(user.id)}>✅ Activate</button>
                      : <button className="btn btn-danger btn-sm"  onClick={() => blockUser(user.id)}>🚫 Block</button>
                    }
                  </td>
                </tr>
              ))}
