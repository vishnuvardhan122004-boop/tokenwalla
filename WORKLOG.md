...
- **Branch:** `main` tip is still `58b2847` (unchanged by this session). This session's work is on `claude/new-session-liauqa`, **pushed to origin, no PR opened yet**. **Do NOT delete** `develop`, which deploys to staging.
- **Latest commit at last update:** `main` `58b2847` (includes **#84** — item 17 phase 1, the single-use `otp_token`; **#82** — item 20's pagination — merged before this session started, neither had a WORKLOG/ROADMAP entry until the correction pass below). This session's own commits sit ahead of that on `claude/new-session-liauqa`, not yet merged.
- **Last updated:** 2026-09-11 (second slice) — **the receipt endpoint finally has a caller — web half.** `BookingReceiptView` (`GET /api/payment/receipt/<pk>/`) existed since 2026-08-11 with nothing calling it — the `## Next` item flagged it as "the most substantive product gap open" for a paid healthcare service in India. `MyBookings.js` now shows a **Receipt** link on every booking with `amount > 0`, opening a modal that fetches the GST breakdown and renders: seller/GSTIN, receipt number, booking details, an itemised table (doctor fee marked GST-exempt with its note, platform + gateway fee at 18%), the taxable-value/GST/total summary, and a pass note when the visit bought or redeemed an Appointment Pass. A **Print / Save as PDF** button calls `window.print()` — no PDF-generation dependency added.
  - **The print CSS needed three real iterations, worth recording in full.** First pass (`body * { visibility: hidden }`, opt the receipt back in) left content cut off — `.mb-receipt-modal`'s own `max-height:85vh; overflow-y:auto` was clipping the absolutely-positioned receipt exactly as it would a scrolled view. Second pass reset the modal/overlay to `position:static` for print and switched to hiding `.mb-root` via `display:none` — correctly removed this page's own header/tabs (confirmed via `getComputedStyle`) but left the global Navbar and Footer's own text still printing. Root cause: Bootstrap ships a global (not print-scoped) `.visible { visibility: visible !important }` utility class, and a bare `body *` selector has lower specificity than any single class selector, so it lost regardless of `!important` on both sides. Final fix names the Navbar/Footer directly (`nav.nav-root`, `footer.footer-root` — tag+class beats a lone class on specificity) on top of the `.mb-root` approach. Verified against the real output that matters — `page.pdf()`, not just a screenshot — which dropped from 496KB (leaking the footer's Play Store badge image) to 63KB once actually clean.
  - **Two self-inflicted detours, both worth remembering.** (1) A literal backtick typed inside a CSS comment — twice — silently broke the JS parse of the component's own `<style>{\`...\`}</style>` template literal; webpack kept serving the last-good bundle on both occasions, so two full rounds of browser verification were run against stale code before the `webpack compiled with 2 errors` line in the dev-server log was actually read. Any inline `<style>{\`...\`}</style>` block in this codebase can never contain a literal backtick, comments included. (2) The local Django test/dev database needs `python manage.py createcachetable` after `migrate` — `migrate` alone doesn't create it, and the login endpoint's `AnonRateThrottle` reads the cache, so login 500'd with `no such table: tw_cache_table` until this was run. Recorded in CLAUDE.md's browser-verification recipe so the next session doesn't lose time to either.
  - **Verified end-to-end in an actual browser, not just test-client assertions**, per the browser-verification rule this session's earlier slice added to CLAUDE.md: seeded a real patient/doctor/booking/payment via the Django ORM (`compute_fee_breakdown`, not hand-typed numbers), logged in through the real `/login` form, opened `/my-bookings`, opened the receipt modal, and confirmed both a full-page screenshot and a real generated PDF render the complete, correctly-computed breakdown with zero page chrome leaking through.
  - 6 new tests in `MyBookings.receipt.test.js`: the link shows only when `amount > 0`, the fetched breakdown renders, a redeemed-pass visit shows the credit note instead of a bare ₹0, a missing-Payment-row 404 surfaces the server's own message rather than a generic failure, and Close dismisses the modal. **67 frontend** (was 61, +6), **557 backend unchanged** (no backend file touched this slice), `makemigrations --check` not applicable (no model change).
  - **App half not done** — separate repo (`tokenwalla.app`), separate release cycle, out of scope for a web-only session. `## Next`'s bullet now reads accordingly.
  - Pushed on `claude/new-session-liauqa` (no PR opened, not asked for).
- **Previously:** 2026-09-11 (first slice) — **`/start` correction pass: item 17 phase 1 and two `## Next` bugs were already fixed and undocumented; OTP/login rate-limit visibility shipped.** Walking `## Now` top to bottom (same shape as the two 2026-09-09 audits below) found every numbered item done, Vishnu-blocked (4c, 13, 14b, 16 — Railway/Razorpay dashboard access) or Meta-blocked (9, 14c) — but also found the file itself had drifted: `58b2847` (PR #84, 2026-09-10) shipped item 17 phase 1 with no ROADMAP/WORKLOG update at all, and two `## Next` bullets described bugs already fixed days earlier (the web signup password regex, closed the same day it was filed by `b2d5dbe` 2026-09-06; the guard's `origin/main` false-positive, closed 2026-09-09 as item 18) but never struck through. All three corrected in ROADMAP directly rather than left to mislead the next session — see item 17 and the `## Next` list there for the exact wording.
  - **The actual slice, once the audit was done: OTP/login rate-limit visibility.** `RateCounter` — the table backing the OTP-per-mobile send cap (**10/day**, hardcoded in `users/auth_views.py`, the real SMS-spend control), the OTP-per-IP send cap (2000/day, `NUM_PROXIES=1` still means this counts a rotating Railway edge IP rather than the real caller — item 16, still Vishnu's), the OTP wrong-guess attempt cap (5), and the login-failure cap — had **no admin registration anywhere**, confirmed by grep before writing anything, matching the same gap the `## Next` list flagged for it 2026-08-14 ("A blocked OTP IP is invisible"). `users/admin.py:RateCounterAdmin` now lists every row ordered by count descending (whatever is closest to tripping its limit is on screen first), filterable by kind via a `SimpleListFilter` that reads the distinct prefixes actually in the table (`otp_sends`, `otp_sends_ip`, `otp_attempts`, `login_fails`) rather than hard-coding them, searchable by key — same shape as the existing `WhatsAppLogAdmin` in `notifications/admin.py`. Read-only in spirit (no permission overrides added, matching that same admin class's style); a superuser can still delete a row to manually clear a wrongly-tripped cap, same effect as the existing `RateCounter.reset()` classmethod.
  - **Also found while scoping this: `notifications/admin.py`'s `WhatsAppLogAdmin` already existed** — a `## Next` bullet ("Backend observability") still described it as missing. Corrected in place rather than left stale; the broader claim in that bullet (no APM/error-tracking for the Django API itself) is still true and stays open.
  - Local test environment needed rebuilding from scratch this session (Django wasn't installed, then a broken system `cryptography`/`cffi` install crashed every JWT-touching test with a Rust panic on import) — fixed with a clean `pip install cffi cryptography` and running against `DATABASE_URL=sqlite:///db.sqlite3` (the project's own local/test setup, per the guard hook's own remedy text) rather than Postgres. Environment-only; nothing here is a code change.
  - 5 new tests in `users/tests_rate_counter.py` (`RateCounterAdminTests`): changelist loads and shows a row, the kind filter narrows correctly, the filter only offers prefixes actually present, the `kind` display method reads the prefix, ordering is count-descending. **557 backend tests (2 skipped)** — was 540 at the last recorded baseline, +12 undocumented from item 17's `tests_otp_token.py` +5 from this session. **61 frontend**, unchanged (no frontend file touched). `makemigrations --check` clean (no model change).
  - **Session close:** pushed as `18a20f7` (docs) + `0254b78` (feature) on `claude/new-session-liauqa` — `git push -u origin claude/new-session-liauqa` succeeded, **no PR opened** (not asked for). `## Next` reordered: with this bullet closed and the other two confirmed already-fixed, **"Nothing consumes the receipt endpoint" is now the top of `## Next`** — verified today it's still genuinely open (`bi-receipt` in `Payment.js` is an icon class, not a caller of `BookingReceiptView`), recommended as tomorrow's first move.
