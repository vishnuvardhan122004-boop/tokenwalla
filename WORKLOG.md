...
- **Branch:** website on `main` (tip `58b2847`, PRs #82 and #84 both merged since the last entry below). This session works on `claude/new-session-liauqa`, cut from that tip. **Do NOT delete** `develop`, which deploys to staging.
- **Latest commit at last update:** website `58b2847` `main` (includes **#84** — item 17 phase 1, the single-use `otp_token`) · previously `b3a10b3` (**#82** — item 20's pagination, merged since the entry below was written). Both merged and live; neither had a WORKLOG/ROADMAP entry until this session's correction pass below.
