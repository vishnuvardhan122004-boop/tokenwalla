...
- **Branch:** website `main` tip `be52c21` — **PR #94 IS merged** (contrary to the line below, which was written before Vishnu merged it and never updated; `be52c21` "Surface the real reason a signup password is rejected (#94)" is the current tip of `origin/main`). App PR #27 is merged too (`7a2e66d` is the tip of `tokenwalla.app` `main`). Today's work is on `docs/item-17-close-out`. Previously tip `7bffcc4` (item 23 fully closed — PR #92 then PR #93, both merged 2026-09-16; also carries PRs #89–#91 from 2026-09-11, none of this session's doing). This session's password-error fix is on `fix/register-password-error-surfacing`, branched clean off `7bffcc4`, in **both** `tokenwalla` and `tokenwalla.app` (same bug, same fix, both repos) — the app repo was newly attached to this session for it. Item 23's docs close-out is a separate pending branch, `docs/item-23-close-out`. **Do NOT delete** `develop`, which deploys to staging.
