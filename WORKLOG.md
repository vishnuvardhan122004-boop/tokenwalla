...
- **Branch:** website `main` tip `2451b5b` — **session closed with zero open PRs and a clean tree.** Five merged 2026-09-17: **#95** (item 17 status correction), **#96** (item 23 close-out), **#86** (Pending Payout caption), **#97** (items 16 + 13 closed), **#98** (eas submit + the Codex P1 fix); **#94** landed just before. App `main` is `7a2e66d` (app PR #27 merged), **not built since v1.4.0 / versionCode 40 on 2026-08-29** — that build is tomorrow's first move. **Do NOT delete** `develop`, which deploys to staging. Previously tip `7bffcc4` (item 23 fully closed — PR #92 then PR #93, both merged 2026-09-16; also carries PRs #89–#91 from 2026-09-11, none of this session's doing). This session's password-error fix is on `fix/register-password-error-surfacing`, branched clean off `7bffcc4`, in **both** `tokenwalla` and `tokenwalla.app` (same bug, same fix, both repos) — the app repo was newly attached to this session for it. Item 23's docs close-out is a separate pending branch, `docs/item-23-close-out`. **Do NOT delete** `develop`, which deploys to staging.
- **Latest commit at last update:** website `2451b5b` on `main`, all merged and deployed. App repo `7a2e66d` on `main`, merged but **unbuilt** — the source carries the OTP nonce on all six call sites and no phone is running it. Item 20's pagination (#82) is confirmed on `main` (`b3a10b3`).
- **Last updated:** 2026-09-17 (**session wrap**) — **five PRs merged, the session's only 🔴 closed, and almost no code written.** `main` `be52c21` → `2451b5b`; zero open PRs; working tree clean. Started from "what work is pending?" and the answer turned out to be that **the docs described a repo that no longer existed**: item 17 called shipped work unstarted, item 23 called merged PRs pending, PR #94 was recorded as unmerged while sitting on `main`, a branch had been pushed for days with no PR, and PR #96 showed a green check that was a day-old **Vercel preview** rather than the test suite. Closed **16** (`NUM_PROXIES=2`, verified by a stable `resolved_ident` across three reloads — the security item that had every per-IP throttle keyed to a rotating Railway edge address), **13** (`APP_LATEST_VERSION=1.4.0` live), **17 → 🟢** (the OTP nonce was already on backend + web + app source), **23** (close-out), and merged **#86** (Pending Payout caption, plus a second inaccuracy the fix itself introduced). **A Codex P1 on #98 caught the one genuinely dangerous thing:** the build sequence treated a *dismissible* update nag as proof that legacy installs were gone, which would have justified retiring the OTP fallback and locking real patients out of registration and password reset — corrected in four places, with the actual gate (instrument calls arriving without an `otp_token`) written down as item 17's next step. **Total logic changed all day: one JSX string.** 574 backend (2 skipped) / 63 frontend green throughout; `makemigrations --check` clean. Tomorrow: the EAS build (see ROADMAP's top block for the full sequence and the `play-service-account.json` precondition). Detail of the last slice follows.
