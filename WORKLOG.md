...
- **Latest commit at last update:** website `c0d7c28` on `claude/admin-fraction-hospital-automation-neoqnl`, **pushed, not merged** — item 23 (admin bookings-by-location + hospital auto-close-stale-bookings). `main` itself is still `06e2e87`. Item 20's pagination PR (#82, `60059d9`) status not re-checked this session — see item 20 in ROADMAP for its last known state.
- **Last updated:** 2026-09-16 — **new item 23: admin bookings-by-location report + hospital auto-close-stale-bookings, pushed, not merged.** Two features requested together in one session. (1) `AdminReportsView` gains an additive `by_location` key — bookings grouped on `Hospital.city`, ranked by volume with count + fraction, computed over the whole table not just the 500-row `recent` slice; `src/ADMIN/Reports.js` renders it as a ranked bar list. Read-only, no migration. (2) New `bookings.management.commands.close_stale_bookings` (meant for a ~15-min Railway Cron, not yet wired up): an `IN_PROGRESS` booking over 2h old auto-completes, a `CONFIRMED` booking never called within 2h of its slot auto-no-shows; `ON_HOLD` is deliberately never touched. New nullable `Booking.called_at` (migration `0014`) is the discriminator, stamped by `CallNextView` and the QR-scan endpoint — the only two places a booking becomes `IN_PROGRESS`. `_claim_transition` grew an `**extra_fields` param so `called_at` rides the same atomic conditional `UPDATE`; both cron sweeps use the identical "conditional update wins" idiom so a live staff action always beats the cron. **Three design questions were asked and answered by Vishnu before any code was written** (this touches the exact statuses `run_daily_payouts` watches): the 2h timer applies to `IN_PROGRESS` only, not `CONFIRMED` (a delayed doctor can leave someone waiting near 2h with no fault of staff); a never-called `CONFIRMED` booking becomes `NO_SHOW` not `COMPLETED` (no payout implied for an unconfirmed visit); ships as a real cron, not a lazy dashboard-load check. **This is not payout automation** — `NO_SHOW` already feeds `run_daily_payouts` today (pre-existing); marking a doctor paid stays 100% manual. **Flagged, not fixed:** the first cron run after deploy will auto-no-show (with the same push/WhatsApp `NoShowView` sends) any `CONFIRMED` booking from the last 7 days that was never called and whose slot already passed — a one-time catch-up, not a bug. 571 backend tests (2 skipped, was 556 — 15 new: `bookings/tests_close_stale_bookings.py`, `payments/tests_admin_reports.py`), 61 frontend unchanged, `makemigrations --check` clean, money-path suites (`tests_payments`/`tests_integration`/`tests_pass`) re-run green, `-v 2` run has zero `graph.facebook.com` lines. No `/api/payment/*`/`/api/bookings/*` contract change — `by_location` is additive and admin-only, `called_at` is DB-only and not on any serializer. `/ship` gate: SHIP. Pushed as `claude/admin-fraction-hospital-automation-neoqnl` @ `c0d7c28`. **Not done:** the Railway Cron Schedule itself (Vishnu's, same hand-off as 14b's two crons), and no PR opened (not asked for this session). **Also found, not this session's work:** three small fixes landed on `main` 2026-09-11 (PRs #89–#91 — scan/blood-test fee labelling, dev-server Cloudinary/WhatsApp safety, orphaned hospital image cleanup on replace/delete) that were never written up in this file or ROADMAP; noted here so the gap is visible rather than silently skipped.
