...
- **Branch:** website `main` tip `be52c21` (PR #94 — the password-error fix — merged 2026-09-16 on top of item 23's `7bffcc4`; also carries PRs #89–#91 from 2026-09-11, none of this session's doing). App repo (`tokenwalla.app`, newly attached to this session) `main` tip `7a2e66d` (PR #27, the matching fix, merged same day). This session's ROADMAP/WORKLOG wrap is on `docs/register-password-fix-merged`, **not yet a PR**; two earlier docs branches from the same session (`docs/item-23-close-out`, an intermediate pass superseded by this one) were never merged and are safe to delete — nothing on them isn't already captured here. **Do NOT delete** `develop`, which deploys to staging.
- **Latest commit at last update:** website `main` `be52c21` (item 23 + item 24, both merged); app repo `main` `7a2e66d` (item 24's app half, merged) — plus `docs/changelog-1.5.0` on the app repo, pushed, **not merged**, adding `CHANGELOG.md` for the pending v1.5.0 build (item 25). Item 20's pagination PR (#82, `60059d9`) status not re-checked this session — see item 20 in ROADMAP for its last known state.
- **Last updated:** 2026-09-16 (session close) — **item 23 closed, item 24 (a real cross-repo auth bug) found and fixed, item 25 (mobile app v1.5.0) made build-ready. Full detail in ROADMAP; this file's job is proof, not narrative:**
  - **Item 23** (admin bookings-by-location + hospital auto-close-stale-bookings): 574 backend tests (2 skipped, was 556), 61 frontend unchanged, `makemigrations --check` clean, money-path suites re-run green, `-v 2` run zero `graph.facebook.com` lines. Merged PR #92 (`main` `97041e2`) then PR #93 (`main` `7bffcc4`).
  - **Item 24** (signup hides the real password-rejection reason, web + app): web 63 frontend tests (was 61, +2), build clean, backend untouched; app `tsc`/`expo lint` clean, 174/174 tests unchanged. Merged web PR #94 (`main` `be52c21`) and app PR #27 (`tokenwalla.app` `main` `7a2e66d`).
  - **Item 25** (ship app v1.5.0): no tests to run — `CHANGELOG.md` is a new, docs-only file, `app.json`/`package.json` untouched (already `1.5.0`). Pushed as `docs/changelog-1.5.0` in `tokenwalla.app`, not merged, no PR opened.
  - Full narrative for all three below, unchanged from when each landed today.
- **Previously (same day):** 2026-09-16 — **new item 25: mobile app v1.5.0 made build-ready, not yet built.** Following on from item 24, asked to prep the app for its next release. Version was **already** `1.5.0` in `app.json`/`package.json` — Vishnu's own `chore(release)` commit `b8ee37c`, cut 2026-09-15 (the day before this session), with a descriptive commit-message body as this repo's only prior "changelog" convention. Two real fixes landed *after* that commit was cut, before any build ran, and neither made it into that note: R8/ProGuard obfuscation (`c0a6788`) and item 24's password-error fix (`#27`, merged this session). Added `CHANGELOG.md` — a new file, first of its kind in this repo — carrying the complete, accurate 1.5.0 list forward so it survives past one now-stale commit message. Pushed as `docs/changelog-1.5.0` on `tokenwalla.app`, **not merged, no PR opened**. **Explicitly not done, and not a session's to do:** running `eas build` itself (no EAS credentials in a session), `eas submit`, and — only once the store has actually approved the build — setting `APP_LATEST_VERSION=1.5.0` on Railway (a env var outside the `PASS_ENABLED` carve-out, and premature before then per ROADMAP item 5b's own documented lesson). Full detail in ROADMAP item 25.
- **Previously (same day):** 2026-09-16 — **the signup form on both web and app was hiding the server's real password-rejection reason — found and fixed on both repos, merged.** Asked to check whether the mobile app had the same alphanumeric-only password regex bug the web form once had (item below, already fixed and closed while checking — confirmed still on `main` at `b2d5dbe`, nothing re-fixed). The app never had that bug (its client check is only `length >= 6`, no character-class regex anywhere) — but that same length-only check meant a password like `123456` could reach the server and get rejected by Django's `CommonPasswordValidator`/`NumericPasswordValidator`/similarity check, and neither client actually showed the reason. Root cause: `RegisterSerializer.validate()` raises a field-keyed DRF error (`{password: [...]}`) for that rejection — not the `{message}` shape every other auth endpoint here returns — and neither `profilecreate.js`'s nor `register.tsx`'s catch block checked for a `password` key (web already had this exact pattern for a duplicate `mobile`, just never extended it to `password`; the app checked neither). Fixed both catch blocks to read the field-keyed error, and added one cheap client-side pre-check on both platforms — reject an all-numeric password before the round trip, matching `NumericPasswordValidator` exactly. Deliberately did not try to duplicate `CommonPasswordValidator` (a large word list) or the similarity check client-side; the message fix means a round trip for those now surfaces the real reason instead of a generic failure. Web: 2 new regression tests (`profilecreate.test.js`), 63 frontend tests (was 61), build clean. App: newly cloned into this session (`vishnuvardhan122004-boop/tokenwalla.app`, not previously attached), `tsc --noEmit` and `expo lint` clean, 174/174 tests unchanged (no screen-test infra exists for this file, a pre-existing gap). Backend untouched on both. Pushed as `fix/register-password-error-surfacing` in both repos, **both merged 2026-09-16**: web PR #94 (`main` `be52c21`, CI green — Backend tests, Website tests & build — before merge, no review threads; Vercel/Railway redeployed automatically, live now) and app PR #27 (`tokenwalla.app` `main` `7a2e66d`, no CI configured in that repo, `mergeable_state: clean`). The app half is code-merged only — an EAS build + store review still stand between this and any patient's phone.
