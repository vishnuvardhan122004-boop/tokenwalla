...
- **Latest commit at last update:** website `f5f35ff` on `claude/railway-cron-config-followup`, **pushed, PR #93 open** — the two Railway cron config files. Item 23 itself (`97041e2`) is merged. Item 20's pagination PR (#82, `60059d9`) status not re-checked this session — see item 20 in ROADMAP for its last known state.
- **Last updated:** 2026-09-16 — **item 23 merged as PR #92; a bot-review follow-up opened as PR #93.** Two features requested together in one session. (1) `AdminReportsView` gains an additive `by_location` key — bookings grouped on `Hospital.city`, ranked by volume with count + fraction, computed over the whole table not just the 500-row `recent` slice; `src/ADMIN/Reports.js` renders it as a ranked bar list. Read-only, no migration. (2) Auto-close a stale booking — **split into two separate commands on two separate schedules, corrected mid-session:** `bookings.management.commands.close_stale_bookings` (every ~15 min) auto-completes an `IN_PROGRESS` booking 2h after it was called in; `bookings.management.commands.mark_daily_no_shows` (once daily, shortly after midnight) auto-no-shows a `CONFIRMED` booking never called once its whole day — not a rolling 2h window — has ended. The first cut of this used one rolling 2h clock for both cases; Vishnu's explicit correction was that a delayed doctor can leave a patient genuinely still waiting near 2h with nobody at fault, so "never showed" is only unambiguous once the clinic day is actually over, hence the daily-after-midnight schedule for the no-show side specifically. `ON_HOLD` is deliberately never touched by either. New nullable `Booking.called_at` (migration `0014`) is the discriminator both key on, stamped by `CallNextView` and the QR-scan endpoint — the only two places a booking becomes `IN_PROGRESS`. `_claim_transition` grew an `**extra_fields` param so `called_at` rides the same atomic conditional `UPDATE`; both commands use the identical "conditional update wins" idiom so a live staff action always beats the sweep. **This is not payout automation** — `NO_SHOW` already feeds `run_daily_payouts` today (pre-existing); marking a doctor paid stays 100% manual. **Flagged, not fixed:** the first midnight `mark_daily_no_shows` runs after deploy will auto-no-show (with the same push/WhatsApp `NoShowView` sends) any `CONFIRMED` booking from the last 7 days that was never called — a one-time catch-up, not a bug. 574 backend tests (2 skipped, was 556 — 18 new: `bookings/tests_close_stale_bookings.py`, `bookings/tests_mark_daily_no_shows.py`, `payments/tests_admin_reports.py`), 61 frontend unchanged, `makemigrations --check` clean, money-path suites (`tests_payments`/`tests_integration`/`tests_pass`) re-run green, `-v 2` run has zero `graph.facebook.com` lines. No `/api/payment/*`/`/api/bookings/*` contract change — `by_location` is additive and admin-only, `called_at` is DB-only and not on any serializer. `/ship` gate: SHIP. **Merged as PR #92** (squash, `97041e2`, merged by Vishnu at 13:25:46 UTC).
  - **PR #93, open**: a bot review (`chatgpt-codex-connector`) on #92, posted at 13:25:35 UTC — 11 seconds before the merge — correctly flagged that neither new command had any cron wiring anywhere in the repo. Fixed with two new Railway Config-as-Code files (`backend/railway.close-stale-bookings.cron.json` at `*/15 * * * *`, `backend/railway.daily-no-shows.cron.json` at `45 18 * * *` UTC = 00:15 IST), matching the two existing crons' exact shape. Still needs the two Railway *services* themselves created and pointed at these files — a dashboard step, Vishnu's, same hand-off as item 14b.
